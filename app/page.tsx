...
export default function BrixsportsWaitlist() {
  ...
  useEffect(() => {
    ...
    // Enhanced referral source tracking
    const captureReferralSource = () => {
      // Check for URL parameters first
      const urlParams = new URLSearchParams(window.location.search);
      const urlRef = urlParams.get('ref') || urlParams.get('source');
      
      // If we have a URL ref, use it
      if (urlRef) {
        setReferralSource(urlRef);
        return;
      }
      
      // Check if we have a referrer (user came from another site)
      if (document.referrer) {
        try {
          const referrerUrl = new URL(document.referrer);
          // Map common referrers to friendly names
          if (referrerUrl.hostname.includes('snapchat')) {
            setReferralSource('snapchat');
            return;
          } else if (referrerUrl.hostname.includes('instagram')) {
            setReferralSource('instagram');
            return;
          } else if (referrerUrl.hostname.includes('tiktok')) {
            setReferralSource('tiktok');
            return;
          } else if (referrerUrl.hostname.includes('twitter') || referrerUrl.hostname.includes('x.com')) {
            setReferralSource('twitter');
            return;
          } else if (referrerUrl.hostname.includes('whatsapp')) {
            setReferralSource('whatsapp');
            return;
          } else {
            // For other referrers, use the hostname
            setReferralSource(referrerUrl.hostname.replace('www.', ''));
            return;
          }
        } catch (e) {
          // If referrer is not a valid URL, fall through
        }
      }
      
      // Default to direct if no other source detected
      setReferralSource('direct');
    };

    // Capture referral source on initial load
    captureReferralSource();

    // Set up a periodic check for referral source changes
    // This helps capture referrals when users navigate back from social media
    const referralCheckInterval = setInterval(captureReferralSource, 1000);
    ...
    const statsInterval = setInterval(fetchWaitlistStats, 30000);

    return () => {
      window.removeEventListener('scroll', handleScroll);
      clearInterval(referralCheckInterval);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-green-950 relative overflow-hidden">
      {/* Minimal Header - Option 1 */}
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled 
          ? 'bg-slate-950/80 backdrop-blur-md py-4 px-6 shadow-lg' 
          : 'py-4 px-6'
      }`}>
        <div className="container mx-auto">
          <div className="flex items-center justify-between">
            ...
            <Button 
              ...
              className="bg-gradient-to-r from-blue-500 via-green-500 to-blue-600 hover:from-blue-600 hover:via-green-600 hover:to-blue-700 text-white font-black py-2 px-4 text-sm transition-all duration-300 hover:scale-105 shadow-lg rounded-xl"
            >
              Join Waitlist
            </Button>
          </div>
      ...
      {/* Rest of the page content with top padding to account for fixed header */}
      <div className="pt-20">
        {/* Hero Section */}
            ...
            <div className="text-center max-w-6xl mx-auto">
              ...
              <div className="mb-6 md:mb-8 space-y-4 md:space-y-6">
                <div className="space-y-6">
                  ...
                  <a 
                    href="https://chat.whatsapp.com/G7NDG2rASDx29LZaKlQjVK?mode=ems_wa_t&ref=waitlist_banner&utm_source=brixsports&utm_medium=waitlist&utm_campaign=community&utm_content=banner"
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={() => {
                      // Update URL with referral source before navigating
                      const url = new URL(window.location.href);
                      url.searchParams.set('ref', 'whatsapp_banner');
                      window.history.replaceState({}, '', url.toString());
                    }}
                    className="block bg-gradient-to-r from-green-600/30 to-green-800/30 rounded-xl p-3 sm:p-4 border-2 border-green-400/50 hover:from-green-600/40 hover:to-green-800/40 transition-all duration-300 hover:scale-[1.02] cursor-pointer"
                  >
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div>
                        <h3 className="text-sm sm:text-lg md:text-xl font-black text-white">Join Our WhatsApp Community!</h3>
                        <p className="text-green-200 text-xs sm:text-sm font-semibold">Connect with other sports enthusiasts</p>
                      </div>
                      <div className="ml-auto">
                        <ArrowRight className="h-5 w-5 sm:h-6 sm:w-6 text-green-400" />
                      </div>
                    </div>
                  </a>
                  ...
                  {isSubmitted ? (
                    ...
                    <Card className="max-w-lg mx-auto bg-black/70 backdrop-blur-md border-2 border-blue-400/50 shadow-2xl hover:shadow-blue-500/20 transition-all duration-300 mx-4 sm:mx-auto">
                      <CardContent className="p-6 sm:p-8 md:p-10">
                        ...
                        <form id="waitlist-form" onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                          <Input
                            type="text"
        ...
        <section className="relative z-10 py-6 md:py-8 bg-gradient-to-r from-blue-500/10 to-green-500/10">
          <div className="container mx-auto px-4">
            <div className="text-center">
              <h3 className="text-lg sm:text-xl md:text-2xl font-black mb-4 text-white">Follow the Movement</h3>
              <div className="flex flex-wrap justify-center gap-2 sm:gap-3 md:gap-4 max-w-2xl mx-auto">
                <a
                  href="https://snapchat.com/add/brixsports?ref=snapchat&utm_source=brixsports&utm_medium=social&utm_campaign=community&utm_content=snapchat"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-gradient-to-br from-yellow-400 to-orange-500 p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl shadow-lg sm:shadow-2xl hover:scale-105 transition-all duration-300 hover:shadow-yellow-500/30 border border-white/20"
                ...
                <a
                  href="https://instagram.com/brixsports?ref=instagram&utm_source=brixsports&utm_medium=social&utm_campaign=community&utm_content=instagram"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-gradient-to-br from-pink-500 to-purple-600 p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl shadow-lg sm:shadow-2xl hover:scale-105 transition-all duration-300 hover:shadow-pink-500/30 border border-white/20"
                  aria-label="Follow Brixsports on Instagram"
                >
                  <Instagram className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-white" />
                </a>
                <a
                  href="https://tiktok.com/@brixsports?ref=tiktok&utm_source=brixsports&utm_medium=social&utm_campaign=community&utm_content=tiktok"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-gradient-to-br from-black to-red-600 p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl shadow-lg sm:shadow-2xl hover:scale-105 transition-all duration-300 hover:shadow-red-500/30 border border-white/20"
                ...
                <a
                  href="https://x.com/brixsports?ref=twitter&utm_source=brixsports&utm_medium=social&utm_campaign=community&utm_content=twitter"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-gradient-to-br from-gray-800 to-black p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl shadow-lg sm:shadow-2xl hover:scale-105 transition-all duration-300 hover:shadow-gray-500/30 border border-white/20"
                  aria-label="Follow Brixsports on X"
                >
                  <Twitter className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-white" />
                </a>
                <a
                  href="https://chat.whatsapp.com/G7NDG2rASDx29LZaKlQjVK?mode=ems_wa_t&ref=whatsapp&utm_source=brixsports&utm_medium=social&utm_campaign=community&utm_content=whatsapp"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-gradient-to-br from-green-500 to-green-700 p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl shadow-lg sm:shadow-2xl hover:scale-105 transition-all duration-300 hover:shadow-green-500/30 border border-white/20"
                  aria-label="Join Brixsports WhatsApp Community"
                >
                  <svg className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-white" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                  </svg>
                ...
                <a
                  href="mailto:brixsports2025@gmail.com?ref=email&utm_source=brixsports&utm_medium=email&utm_campaign=community&utm_content=email"
                  className="bg-gradient-to-br from-blue-500 to-green-500 p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl shadow-lg sm:shadow-2xl hover:scale-105 transition-all duration-300 hover:shadow-blue-500/30 border border-white/20"
